...
def get_competition_ids(user_token):
    ...
    filename = os.path.abspath(
        f"configs/active_competitions/competition_data.json")
...
def update_trained_competitions(user_token, compe_data):
    trained_competitions(user_token, compe_data, update_last_predicted_at=True)


def trained_competitions(user_token, compe_data, update_last_predicted_at=False):
    # Load existing hyperparameters data
    directory = os.path.abspath("configs/active_competitions/")
    os.makedirs(directory, exist_ok=True)

    filename = os.path.abspath(f"{directory}/trained_competitions.json")
    ...
    if id in trained_compe_data:
        # If the option to update the last predicted timestamp is set
        if update_last_predicted_at:
...
def do_update_trained_competition(user_token, compe_data):
    try:
        ...
        response = requests.post(
            f"{API_BASE_URL}/admin/predictions/from-python-app/update-competition-last-training", data=json_data, headers=headers)
        response.raise_for_status()  # Raise an exception for bad responses (4xx and 5xx)

    except requests.RequestException as e:
        error_message = f"Failed to update trained competition {id}. Error: {e}"
