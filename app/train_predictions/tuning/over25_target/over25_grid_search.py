...
def scorer_v2(estimator, X, y_true, occurrences):
    occ_0 = occurrences.get(0, 0)  # Under 2.5
    occ_1 = occurrences.get(1, 0)  # Over 2.5
    ...
    # Dynamic threshold: at least 5% or 10% of dataset
    max_diff_threshold = max(5, 0.1 * totals)

    # Soft penalties for overshooting distribution
    penalty = 1.0
    ...
    penalty = max(0.5, penalty)  # don’t zero it out

    # Natural distribution alignment
    diff_0 = abs(y_pred_0 - occ_0)
    diff_1 = abs(y_pred_1 - occ_1)
    natural_score = max(0, 1 - (max(diff_0, diff_1) / 100))

    # Core metrics
    f1 = f1_score(y_true, y_pred, average="weighted", zero_division=0)

    # Coverage penalty (if model never predicts Over when it exists)
    if occ_1 > 0 and all(p == 0 for p in y_pred):
        penalty *= 0.5

    # Weighted blend
    score = (
        0.5 * f1 + 0.5 * natural_score
    ) * penalty
...
def scorer(estimator, X, y_true, occurrences):
    ...
    score = 0.4 * f1 + 0.6 * natural_score
