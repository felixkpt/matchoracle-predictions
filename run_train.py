...
from configs.active_competitions.competition_data import trained_competitions

def run_train(user_token, target, hyperparameters, PREDICTION_TYPE, COMPETITION_ID, TRAIN_TO_DATE, TRAIN_FROM_DATE, be_params):
    ...
    parser.add_argument('--random-search', action='store_true', help='Perform random search')
    ...
    is_grid_search = args.grid_search
    is_random_search = args.random_search
    ...
    train_matches, test_matches = load_for_training(
        compe_data, user_token, be_params, per_page=1000, train_ratio=.70, ignore_saved=ignore_saved)
    ...
    compe_data['trained_to'] = TRAIN_TO_DATE
